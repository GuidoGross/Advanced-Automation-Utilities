...
class Move(MouseAction):
    def __init__(self, x: int, y: int, physics: Optional[MousePhysics] = None):
...
def _generate_human_path(initial_x, initial_y, target_x, target_y, steps, inconsistency, speed):
...
def _bezier_point(point_0, point_1, point_2, point_3, progress):
...
def _distorted_ease(progress, distortion_factor):
    base_ease = _ease(progress)
    distortion = distortion_factor * (progress ** 2) * ((1 - progress) ** 2) * 16
    ease = max(0, min(1, base_ease + distortion))
    return ease

def _calculate_tremor(current_x, current_y, target_x, target_y, inconsistency, speed):
    speed_factor = max(1, speed / 500)
