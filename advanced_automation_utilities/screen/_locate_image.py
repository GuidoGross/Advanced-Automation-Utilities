...
from ._screen_utilities import _take_screenshot, _adjust_coordinates_for_region
...
class _LocateImage(_ScreenAction):
    ...
    def execute(self) -> tuple[Optional[int], Optional[int]]:
        ...
        if template is None:
            raise ValueError(f"Failed to read image at '{self.image_path}'. Ensure it is a valid image file.")
